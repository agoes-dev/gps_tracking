...
$hasil3=mysql_query("select SQL_NO_CACHE * from imo where id_perangkat='$_GET[id_perangkat]'");
$data3=mysql_fetch_array($hasil3);
$selisih=$waktu-strtotime($data3['waktu']);
if($selisih>300)
{
	mysql_query("update imo set waktu='$waktu_dummy' where id_perangkat='$_GET[id_perangkat]'");
	...
	$spbit1 = "\x0D";
	$spbit2 = "\x0A";
	$message = $stbit1.$stbit2.$pclen.$ptcno.$imei.$spbit1.$spbit2;
	echo $message."<br/>";
	socket_write($socket, $message, strlen($message)) or die("Could not send data to server\n");
	//$result = socket_read ($socket, 1024) or die("Could not read server response\n");
	socket_close($socket); 
	echo "<div style=font-family:Arial;font-size:12px;>";
	echo "<b>Immobilize OK</b><br/><br/>";
	echo "<form>";
...
{
	echo "<div style=font-family:Arial;font-size:12px;>";
	echo "Capture : <b>Try again in 1 minutes</b><br/><br/>";
	echo "<form>";
	echo "<input type=button value=Close onClick=window.close()>";
	echo "</form>";
	echo "</div>";
